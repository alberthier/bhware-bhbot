...
from statemachine import Timer
...
class TakeFruits(statemachine.State):
    ...
    def on_enter(self):
        ...
        #yield Trigger(FRUITMOTH_FINGER_OPEN) # for 4 fingers
        yield Trigger(FRUITMOTH_FINGER_RETRACT) # for 2 fruits
...
class NinjaaaaaaaaYellowS(statemachine.State):
    ...
    def on_enter(self):
        ...
        yield Timer(500) # STUPIDTIMER
        yield Trigger(TORCH_GUIDE_HIDE)
        yield MoveLineTo(self.goal.x, self.goal.y, DIRECTION_BACKWARDS)
        self.exit_reason = GOAL_DONE
        yield None
