...
static void zone_get_center(Zone* self, float* x, float* y)
{
    int i = 0;
...
{
    if (!PyArg_ParseTuple(args, "ffffff", &self->field_x1,
                                          &self->field_y1,
                                          &self->field_x2,
                                          &self->field_y2,
                                          &self->zone_escape_increment,
                                          &self->zone_escape_max_increment)) {
        return -1;
    }
...
static PyObject* pathfinder_find_path(PathFinder* self, PyObject* args)
{
    ...
    for (i = 0; i < self->zones_count; ++i) {
        Zone* zone = self->zones[i];
        if (zone->enabled && pathfinder_move_node_ouside_of_zone(self, zone, self->start_node)) {
            pathfinder_synchronize(self);
            break;
        }
    }
