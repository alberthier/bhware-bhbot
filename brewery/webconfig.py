...
def setup_nginx_config_save():
    host_config_root = os.path.join(os.path.dirname(__file__), "web")
    host_config_filename = "nginx-custom-{}.conf".format(gethostname())
    host_config_file = os.path.join(host_config_root, host_config_filename)
    host_config_file_default = "nginx-custom-default.conf"
    host_config_file_target = os.path.join(host_config_root, "nginx-custom.conf")
    ...
    try:
        shutil.copy(host_config_file, host_config_file_target)
...
def setup_nginx_config():
    ...
    host_config_file = os.path.join(host_config_root, host_config_filename)
    host_config_file_default = "nginx-config-default.json"
